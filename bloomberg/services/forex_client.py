...
def get_rates() -> list[dict]:
    ...
    try:
        ...
        for base, quote in PAIRS:
            try:
                if base == "EUR":
                    rate = eur_rates.get(quote, 0)
                elif quote == "EUR":
                    rate = 1.0 / eur_rates.get(base, 1)
                else:
                    rate = eur_rates.get(quote, 0) / eur_rates.get(base, 1)
                result.append({
                    "pair": f"{base}/{quote}",
                    "rate": round(rate, 4),
                    "error": None,
        ...
        # Add BTC/USD via yfinance
        try:
            import yfinance as yf
            btc = yf.Ticker("BTC-USD")
            btc_price = btc.fast_info.last_price
            result.append({"pair": "BTC/USD", "rate": round(float(btc_price), 2), "error": None})
        except Exception:
